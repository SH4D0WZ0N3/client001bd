...
async def _run_historical_scan_with_userbot() -> None:
    """
    Uses a Pyrogram user account (StringSession) to read message history
    from the source channel and queue all messages from START_MESSAGE_ID.

    This bypasses the BOT_METHOD_INVALID restriction since user accounts
    are allowed to call messages.GetHistory.
    """
    from pyrogram.types import StringSession  # type: ignore[attr-defined]
...
async def initial_channel_scan(client: Client) -> None:
    """
    Performs historical scan if:
      1. USER_SESSION_STRING is configured (userbot available)
      2. State does not already record a scan beyond START_MESSAGE_ID

    If no session string is set, skips silently — live messages will be
    queued by the event handler as they arrive.
    """
    ...
    if not settings.USER_SESSION_STRING:
        logger.warning(
            ...
            "To queue existing content, run generate_session.py locally "
            "and set USER_SESSION_STRING in Railway environment variables."
        )
        # Initialize state baseline so the system has a valid document
        if state is None:
